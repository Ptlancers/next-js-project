...
class ReceiptDB:
    ...
    def get_receipt_by_receipt_no(cls, receipt_no: str, section_code: str) -> dict:
        res = cls.collection.find_one({"receipt_number": receipt_no, "section_code": section_code}, {"_id": 0})
        if res:
            res["id"] = str(res["_id"])
            del res["_id"]
        return res
    ...
    def get_all_receipts(cls, section_code: str) -> Generator:
        cursor: Cursor = cls.collection.find({"section_code": section_code}, {"_id": 0})
